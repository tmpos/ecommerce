  ...
  <div class="flex flex-col md:flex-row gap-8">
    ...
    <aside class="w-full md:w-48 shrink-0">
      ...
      <div class="flex flex-col gap-1 mb-4">
        ...
        <a href="/shop<?= $catQuery ? '?' . $catQuery : '' ?>" class="px-3 py-2 rounded-lg text-sm <?= !$currentCategory ? 'bg-primary text-white' : 'hover:bg-input' ?>">
          <?= __('shop_all') ?>
        </a>
        <?php foreach ($categories as $cat): ?>
          ...
          <a href="/shop?<?= http_build_query($catParams) ?>" class="px-3 py-2 rounded-lg text-sm <?= $currentCategory === $cat['slug'] ? 'bg-primary text-white' : 'hover:bg-input' ?>">
            <?= escape($cat['name']) ?>
          </a>
        <?php endforeach; ?>
      </div>
      ...
      <!-- Gender filter -->
      <div class="mb-4 p-3 bg-card rounded-lg border border-color">
        <h4 class="text-sm font-semibold mb-2 text-heading"><?= __('shop_gender') ?></h4>
        <div class="flex flex-col gap-1">
          ...
          <a href="/shop?<?= http_build_query($baseGender) ?>" class="px-2 py-1 text-xs rounded <?= !$currentGender ? 'bg-primary text-white' : 'hover:bg-input' ?>"><?= __('shop_all') ?></a>
          <?php foreach (['men' => __('shop_men'), 'women' => __('shop_women'), 'unisex' => __('shop_unisex')] as $val => $label): ?>
            <a href="/shop?<?= http_build_query(array_merge($baseGender, ['gender' => $val])) ?>" class="px-2 py-1 text-xs rounded <?= $currentGender === $val ? 'bg-primary text-white' : 'hover:bg-input' ?>"><?= $label ?></a>
          <?php endforeach; ?>
        </div>
      </div>
      ...
      <!-- Price range -->
      <div class="mb-4 p-3 bg-card rounded-lg border border-color">
        <h4 class="text-sm font-semibold mb-2 text-heading"><?= __('shop_price_range') ?></h4>
        <form method="GET" action="/shop" class="flex flex-col gap-2">
          ...
          <input type="number" name="min_price" placeholder="<?= __('shop_min') ?>" value="<?= escape($currentMinPrice) ?>" class="w-full border border-input rounded px-2 py-1 text-xs bg-card text-heading" step="0.01" min="0">
          <input type="number" name="max_price" placeholder="<?= __('shop_max') ?>" value="<?= escape($currentMaxPrice) ?>" class="w-full border border-input rounded px-2 py-1 text-xs bg-card text-heading" step="0.01" min="0">
      ...
      <div class="mb-4 p-3 bg-card rounded-lg border border-color">
        <h4 class="text-sm font-semibold mb-2 text-heading"><?= __('shop_size') ?></h4>
        <div class="flex flex-wrap gap-1.5">
          ...
          <?php foreach ($uniqueSizes as $size): ?>
            <a href="/shop?<?= http_build_query(array_merge($sizeBase, ['size' => $size])) ?>" class="px-2.5 py-1 text-xs font-medium rounded border <?= $currentSize === $size ? 'bg-primary text-white border-primary' : 'border-input text-body hover:bg-input' ?>">
              <?= escape($size) ?>
            </a>
          <?php endforeach; ?>
          <?php if ($currentSize): ?>
            <a href="/shop?<?= http_build_query($sizeBase) ?>" class="px-2.5 py-1 text-xs font-medium rounded border border-input text-muted hover:bg-input"><?= __('shop_clear') ?></a>
          <?php endif; ?>
        </div>
      </div>
      <?php endif; ?>
      ...
      <div class="mb-4 p-3 bg-card rounded-lg border border-color">
        <h4 class="text-sm font-semibold mb-2 text-heading"><?= __('shop_color') ?></h4>
        <div class="flex flex-wrap gap-1.5">
          ...
          <?php foreach ($uniqueColors as $color): ?>
            <a href="/shop?<?= http_build_query(array_merge($colorBase, ['color' => $color])) ?>" class="px-2.5 py-1 text-xs font-medium rounded border <?= $currentColor === $color ? 'bg-primary text-white border-primary' : 'border-input text-body hover:bg-input' ?>">
              <?= escape($color) ?>
            </a>
          <?php endforeach; ?>
          <?php if ($currentColor): ?>
            <a href="/shop?<?= http_build_query($colorBase) ?>" class="px-2.5 py-1 text-xs font-medium rounded border border-input text-muted hover:bg-input"><?= __('shop_clear') ?></a>
          <?php endif; ?>
        </div>
      </div>
      <?php endif; ?>
      ...
      <?php if (!empty($brands)): ?>
      <div class="mb-4 p-3 bg-card rounded-lg border border-color">
        <h4 class="text-sm font-semibold mb-2 text-heading"><?= __('shop_brand') ?></h4>
        <div class="flex flex-col gap-1">
          ...
          <a href="/shop?<?= http_build_query($brandBase) ?>" class="px-2 py-1 text-xs rounded <?= !$currentBrand ? 'bg-primary text-white' : 'hover:bg-input' ?>"><?= __('shop_all') ?></a>
          <?php foreach ($brands as $brand): ?>
            <a href="/shop?<?= http_build_query(array_merge($brandBase, ['brand' => $brand['id']])) ?>" class="px-2 py-1 text-xs rounded <?= $currentBrand == $brand['id'] ? 'bg-primary text-white' : 'hover:bg-input' ?>"><?= escape($brand['name']) ?></a>
          <?php endforeach; ?>
        </div>
      </div>
      <?php endif; ?>
      ...
      <div class="mb-4 p-3 bg-card rounded-lg border border-color">
        <h4 class="text-sm font-semibold mb-2 text-heading"><?= __('shop_filters') ?></h4>
    ...
    <div class="flex-1">
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-3">
          <!-- View toggle -->
          <div class="flex border border-input rounded-lg overflow-hidden" data-view-toggle>
            ...
            <button data-view="list" class="p-2 hover:bg-input text-muted" title="<?= __('shop_list_view') ?>">
          ...
          ?>
          <select onchange="window.location='/shop?sort='+this.value<?= $sortExtra ?>" class="text-sm border border-input rounded-lg px-3 py-2 bg-card">
      ...
      <?php if (empty($products)): ?>
        <div class="text-center py-16">
          <svg class="w-20 h-20 mx-auto mb-4 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
          <p class="text-lg text-muted"><?= __('shop_no_products') ?></p>
        </div>
      <?php else: ?>
        <!-- Grid View -->
        <div id="gridView" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <?php foreach ($products as $product): ?>
            ...
            <div class="product-card bg-card rounded-xl overflow-hidden shadow-sm border border-color group">
              ...
              <div class="p-4">
                <p class="text-xs text-muted mb-1"><?= escape($product['category_name'] ?? '') ?></p>
                ...
                <div class="flex items-center gap-2 mb-3">
                  <?php if ($product['sale_price']): ?>
                    ...
                    <span class="text-sm text-muted line-through"><?= $SETTINGS['currency'] ?><?= number_format($product['price'], 2) ?></span>
                  <?php else: ?>
                ...
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <a href="/product/<?= escape($product['slug']) ?>" class="flex items-center gap-1 border border-input text-xs font-medium px-3 py-1.5 rounded-lg hover:bg-input transition">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    <?= __('shop_view') ?>
                  </a>
                  <button data-wishlist="<?= $product['id'] ?>" class="wishlist-btn ml-auto text-muted hover:text-red-500 transition" title="<?= __('shop_wishlist_add') ?>">
        ...
        <div id="listView" class="hidden flex flex-col gap-4">
          <?php foreach ($products as $product): ?>
            <?php $images = json_decode($product['images'] ?? '[]', true); ?>
            <div class="product-card bg-card rounded-xl shadow-sm border border-color flex flex-col md:flex-row overflow-hidden">
              <!-- Image -->
              ...
              <div class="flex flex-col md:flex-row flex-1 p-4 md:p-6 gap-4">
                <div class="flex-1">
                  ...
                  <p class="text-sm text-muted line-clamp-2 mb-3"><?= escape(substr($product['description'] ?? '', 0, 200)) ?></p>
                  <button data-wishlist="<?= $product['id'] ?>" class="wishlist-btn flex items-center gap-1 text-muted hover:text-red-500 transition" title="<?= __('shop_wishlist_add') ?>">
                ...
                <div class="flex md:flex-col items-center md:items-end justify-between md:justify-center gap-3 shrink-0">
                  ...
                  <div class="flex md:flex-col gap-2">
                    ...
                    <a href="/product/<?= escape($product['slug']) ?>" class="flex items-center gap-1.5 border border-input text-sm font-medium px-4 py-2 rounded-lg hover:bg-input transition whitespace-nowrap">
        ...
        <?php if ($totalPages > 1): ?>
          <div class="flex justify-center gap-2 mt-8">
            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
              ...
              ?>
              <a href="/shop?<?= http_build_query($pageParams) ?>" class="px-4 py-2 rounded-lg text-sm <?= $i === $currentPage ? 'bg-primary text-white' : 'bg-card border border-input hover:bg-input' ?>">
                <?= $i ?>
              </a>
            <?php endfor; ?>
          </div>
...
function setView(view) {
  ...
  viewToggle?.querySelectorAll('button').forEach(b => {
    ...
    b.classList.toggle('text-muted', b.dataset.view !== view);
    b.classList.toggle('hover:bg-input', b.dataset.view !== view);
  });
}
...
<div id="cartModal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50">
  <div class="bg-card rounded-2xl p-6 max-w-sm mx-4 shadow-2xl">
    <div class="text-center">
      <div class="w-12 h-12 mx-auto mb-3 rounded-full flex items-center justify-center" style="background:color-mix(in srgb, var(--success) 15%, white);color:var(--success)">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="color:var(--success)"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
      </div>
      <p class="text-sm text-muted mb-1"><?= __('cart_added') ?></p>
      <p id="cartModalName" class="font-semibold text-heading mb-6 truncate"></p>
      <div class="flex flex-col gap-2">
        ...
        <button onclick="document.getElementById('cartModal').classList.add('hidden');document.getElementById('cartModal').classList.remove('flex')" class="w-full text-center border border-input text-body font-medium px-4 py-2.5 rounded-lg hover:bg-input transition"><?= __('cart_continue') ?></button>
      </div>
    </div>
  </div>
</div>
