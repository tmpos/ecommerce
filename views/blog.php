        ...
        <a href="/blog/<?= escape($post['slug']) ?>" style="text-decoration:none;color:inherit;display:block">
          <div class="card" style="overflow:hidden">
            ...
            <div style="padding:1.25rem">
              <p style="font-size:.75rem;color:#94a3b8;margin-bottom:.25rem"><?= date('M j, Y', strtotime($post['published_at'])) ?> &middot; <?= escape($post['author']) ?></p>
              <h2 style="font-size:1.125rem;font-weight:600;margin-bottom:.5rem"><?= escape($post['title']) ?></h2>
              <?php if ($post['excerpt']): ?>
                <p style="font-size:.875rem;color:#64748b"><?= escape($post['excerpt']) ?></p>
              <?php endif; ?>
            </div>
          </div>
        </a>
