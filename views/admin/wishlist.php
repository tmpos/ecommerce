<div class="max-w-7xl mx-auto">
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-muted text-sm"><?= $totalWishlisted ?> total items wishlisted</p>
    </div>
  </div>
  ...
  <div class="card mb-6">
    <div class="card-header flex items-center justify-between">
      ...
      <div class="relative">
        ...
        <svg class="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
      </div>
    </div>
    <div class="card-body" style="padding:0">
      <?php if (empty($products)): ?>
        <p class="text-muted text-center py-12">No wishlist items yet.</p>
      <?php else: ?>
        <div class="table-wrap" style="border:none">
          <table id="productTable">
            <thead>
              <tr>
                <th data-sort="name" data-type="str" class="cursor-pointer hover:text-primary select-none">Product <span class="sort-arrow"></span></th>
                <th data-sort="category" data-type="str" class="cursor-pointer hover:text-primary select-none">Category <span class="sort-arrow"></span></th>
                <th data-sort="price" data-type="num" class="cursor-pointer hover:text-primary select-none">Price <span class="sort-arrow"></span></th>
                <th data-sort="stock" data-type="num" class="cursor-pointer hover:text-primary select-none">Stock <span class="sort-arrow"></span></th>
                <th data-sort="count" data-type="num" class="text-center cursor-pointer hover:text-primary select-none">Wishlists <span class="sort-arrow"></span></th>
              </tr>
            </thead>
                ...
                <tr data-name="<?= strtolower(escape($p['name'])) ?>" data-category="<?= strtolower(escape($p['category_name'] ?? '')) ?>" data-price="<?= $price ?>" data-stock="<?= (int)$p['stock'] ?>" data-count="<?= (int)$p['wishlist_count'] ?>">
                  <td>
                    <div class="flex items-center gap-3">
                      ...
                      <?php else: ?>
                        <div class="w-10 h-10 rounded-lg bg-input flex items-center justify-center">
                          <svg class="w-5 h-5 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        </div>
                      <?php endif; ?>
                      <a href="/product/<?= escape($p['slug']) ?>" class="font-medium hover:text-primary"><?= escape($p['name']) ?></a>
                    </div>
                  </td>
                  ...
                  <td class="text-center">
                    <span class="inline-flex items-center gap-1 font-semibold" style="color:var(--error)">
        ...
        <div class="flex items-center justify-between px-4 py-3 border-t border-color text-sm">
          <span id="productInfo" class="text-muted">Showing <?= count($products) ?> of <?= count($products) ?></span>
    ...
    <div class="card-body" style="padding:0">
      <div class="table-wrap" style="border:none">
        <table id="activityTable">
          <thead>
            <tr>
              <th data-sort="customer" data-type="str" class="cursor-pointer hover:text-primary select-none">Customer <span class="sort-arrow"></span></th>
              <th data-sort="product" data-type="str" class="cursor-pointer hover:text-primary select-none">Product <span class="sort-arrow"></span></th>
              <th data-sort="date" data-type="str" class="cursor-pointer hover:text-primary select-none">Date <span class="sort-arrow"></span></th>
            </tr>
          </thead>
          <tbody id="activityBody">
            <?php foreach ($recent as $r): ?>
              <tr data-customer="<?= strtolower(escape($r['user_name'] . ' ' . $r['user_email'])) ?>" data-product="<?= strtolower(escape($r['product_name'])) ?>" data-date="<?= strtotime($r['created_at']) ?>">
                <td>
                  <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-full bg-primary flex items-center justify-center text-xs font-medium text-white">
                    <?= strtoupper(substr($r['user_name'], 0, 1)) ?>
                    </div>
                    <div>
                      <p class="font-medium"><?= escape($r['user_name']) ?></p>
                      <p class="text-xs text-muted"><?= escape($r['user_email']) ?></p>
                    </div>
                  </div>
                </td>
                <td class="text-body"><?= escape($r['product_name']) ?></td>
                <td class="text-muted text-sm" data-display="<?= date('M j, Y g:i A', strtotime($r['created_at'])) ?>"><?= date('M j, Y g:i A', strtotime($r['created_at'])) ?></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
      ...
      <div class="flex items-center justify-between px-4 py-3 border-t border-color text-sm">
        <span id="activityInfo" class="text-muted">Showing <?= count($recent) ?> of <?= count($recent) ?></span>
        <div class="flex items-center gap-2">
          <button id="actPrevBtn" onclick="changeActPage(-1)" class="btn btn-outline btn-sm" disabled>Prev</button>
          <span id="actPageInfo" class="text-muted"></span>
