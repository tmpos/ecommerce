<div style="display:flex;gap:.5rem;margin-bottom:1.5rem;border-bottom:2px solid var(--border-color);padding-bottom:0;flex-wrap:wrap">
  <?php $tabs = ['sales'=>'Sales', 'inventory'=>'Inventory', 'products'=>'Products', 'customers'=>'Customers']; ?>
  <?php foreach ($tabs as $key => $label): ?>
    <a href="/admin/reports/<?= $key ?>" style="padding:.625rem 1.25rem;font-size:.875rem;font-weight:500;text-decoration:none;color:<?= $tab === $key ? 'var(--primary)' : 'var(--text-muted)' ?>;border-bottom:2px solid <?= $tab === $key ? 'var(--primary)' : 'transparent' ?>;margin-bottom:-2px"><?= $label ?></a>
  <?php endforeach; ?>
</div>
  ...
  <div class="card">
    ...
    <div class="card-body" style="padding-bottom:.5rem">
      ...
      <div style="display:flex;gap:2px;margin-top:4px">
        <?php foreach ($bars as $d): ?>
          <div style="flex:1;font-size:.55rem;color:var(--text-muted);text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= date('j', strtotime($d['day'])) ?></div>
        <?php endforeach; ?>
      </div>
    </div>
  </div>
      ...
      <?php foreach ($paymentMethods as $pm): ?>
        <div style="margin-bottom:.75rem">
          <div style="display:flex;justify-content:space-between;font-size:.8125rem;margin-bottom:4px">
            ...
            <span style="color:var(--text-muted)">$<?= number_format($pm['revenue'], 2) ?> (<?= $pm['count'] ?>)</span>
          </div>
          <div style="background:var(--border-color);border-radius:6px;height:8px;overflow:hidden">
  ...
  <div class="card">
    ...
    <div class="card-body">
      <?php if (empty($topProducts)): ?>
        <p style="color:var(--text-muted);font-size:.875rem">No sales data yet.</p>
      <?php else: ?>
        ...
        <?php foreach ($topProducts as $p): ?>
          <div style="margin-bottom:.75rem">
            <div style="display:flex;justify-content:space-between;font-size:.8125rem;margin-bottom:4px">
              ...
              <span style="color:var(--text-muted)"><?= $p['qty'] ?> &times; $<?= number_format($p['revenue']/$p['qty'], 2) ?></span>
            </div>
            <div style="background:var(--border-color);border-radius:6px;height:8px;overflow:hidden">
              <div style="background:var(--success);height:100%;width:<?= ($p['revenue']/$maxProd)*100 ?>%;border-radius:6px"></div>
            </div>
          </div>
        <?php endforeach; ?>
      <?php endif; ?>
...
<div class="grid-stats">
  ...
  <div class="stat-card">
    ...
    <div class="stat-value" style="color:var(--error)"><?= $outOfStock ?></div>
  </div>
</div>
  ...
  <div class="card">
    ...
    <div class="card-body" style="padding:0">
      <?php if (empty($lowStock)): ?>
        <p style="padding:1.25rem;color:var(--text-muted);font-size:.875rem">All products have sufficient stock.</p>
      <?php else: ?>
      ...
      <?php foreach ($movementSummary as $m): ?>
        <div style="margin-bottom:.75rem">
          <div style="display:flex;justify-content:space-between;font-size:.8125rem;margin-bottom:4px">
            ...
            <span style="color:var(--text-muted)"><?= $m['total_qty'] ?> units (<?= $m['count'] ?> movements)</span>
          </div>
          <div style="background:var(--border-color);border-radius:6px;height:8px;overflow:hidden">
            <div style="background:<?= $m['type'] === 'entry' ? 'var(--success)' : 'var(--error)' ?>;height:100%;width:<?= ($m['count']/$maxMov)*100 ?>%;border-radius:6px"></div>
          </div>
        </div>
      <?php endforeach; ?>
      <?php if (empty($movementSummary)): ?>
        <p style="color:var(--text-muted);font-size:.875rem">No movements recorded.</p>
      <?php endif; ?>
    </div>
  </div>
</div>
  ...
  <div class="card">
    ...
    <div class="card-body">
      <?php if (empty($productsByCategory)): ?>
        <p style="color:var(--text-muted);font-size:.875rem">No categories or products.</p>
      <?php else: ?>
        ...
        <?php foreach ($productsByCategory as $c): ?>
          <div style="margin-bottom:.75rem">
            <div style="display:flex;justify-content:space-between;font-size:.8125rem;margin-bottom:4px">
              ...
              <span style="color:var(--text-muted)"><?= $c['count'] ?> products &middot; avg $<?= number_format($c['avg_price'], 2) ?></span>
            </div>
            <div style="background:var(--border-color);border-radius:6px;height:8px;overflow:hidden">
  ...
  <div class="card">
    ...
    <div class="card-body" style="padding:0">
      <?php if (empty($topRated) || !array_filter($topRated, fn($r) => $r['avg_rating'] > 0)): ?>
        <p style="padding:1.25rem;color:var(--text-muted);font-size:.875rem">No reviews yet.</p>
      <?php else: ?>
        <table style="font-size:.8125rem">
  ...
  <div class="card">
    ...
    <div class="card-body" style="padding-bottom:.5rem">
      ...
      <div style="display:flex;gap:3px;margin-top:4px">
        <?php foreach ($newCustomersMonthly as $m): ?>
          <div style="flex:1;font-size:.55rem;color:var(--text-muted);text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= $m['month'] ?></div>
        <?php endforeach; ?>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="card-body" style="padding:0">
      <?php if (empty($topCustomers)): ?>
        <p style="padding:1.25rem;color:var(--text-muted);font-size:.875rem">No customer orders yet.</p>
      <?php else: ?>
        <table style="font-size:.8125rem">
            ...
            <?php foreach ($topCustomers as $c): ?>
              <tr>
                <td>
                  ...
                  <div style="font-size:.7rem;color:var(--text-muted)"><?= escape($c['email']) ?></div>
                </td>
  ...
  <div class="card-body">
    ...
    <?php foreach ($customerByStatus as $s): ?>
      <div style="margin-bottom:.75rem">
        <div style="display:flex;justify-content:space-between;font-size:.8125rem;margin-bottom:4px">
          <span style="font-weight:500"><?= ucfirst($s['status']) ?></span>
          <span style="color:var(--text-muted)"><?= $s['customers'] ?> customers</span>
        </div>
        <div style="background:var(--border-color);border-radius:6px;height:8px;overflow:hidden">
