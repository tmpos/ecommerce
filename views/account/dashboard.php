<div class="max-w-4xl mx-auto px-4 py-8">
  ...
  <p class="text-muted mb-8"><?= __('account_dashboard') ?></p>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="bg-card rounded-xl p-6 border border-color">
      ...
      <?php else: ?>
      <p id="address-display" class="text-sm text-muted italic"><?= __('account_no_address') ?></p>
      <?php endif; ?>
      ...
      <form id="address-form" method="POST" action="/account/update-address" class="hidden space-y-3 mt-2">
        <div>
          ...
          <input type="text" name="address" value="<?= escape($user['address'] ?? '') ?>" class="w-full border border-input rounded-lg px-3 py-1.5 text-sm bg-card">
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-medium mb-1"><?= __('checkout_city') ?></label>
            <input type="text" name="city" value="<?= escape($user['city'] ?? '') ?>" class="w-full border border-input rounded-lg px-3 py-1.5 text-sm bg-card">
          </div>
          <div>
            <label class="block text-xs font-medium mb-1"><?= __('checkout_zip') ?></label>
            <input type="text" name="zip" value="<?= escape($user['zip'] ?? '') ?>" class="w-full border border-input rounded-lg px-3 py-1.5 text-sm bg-card">
          </div>
        </div>
        <div class="flex gap-2">
          <button type="submit" class="btn-primary text-sm py-1.5 px-4"><?= __('admin_save') ?></button>
          <button type="button" onclick="toggleAddressForm()" class="text-sm px-3 py-1.5 border border-input rounded-lg"><?= __('admin_cancel') ?></button>
        </div>
      </form>
    </div>
    ...
    <div class="bg-card rounded-xl p-6 border border-color">
      ...
      <div class="flex flex-col gap-3">
        <a href="/account/orders" class="flex items-center gap-3 p-3 rounded-lg hover:bg-input transition">
        ...
        <a href="/shop" class="flex items-center gap-3 p-3 rounded-lg hover:bg-input transition">
