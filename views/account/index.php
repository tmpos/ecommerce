<div class="max-w-5xl mx-auto px-4 py-8">
  ...
  <!-- Tabs -->
  <div class="flex gap-1 border-b border-color mb-8">
    <a href="/account/profile" class="tab-btn px-5 py-3 text-sm font-medium rounded-t-lg transition -mb-px border-b-2 <?= $activeTab === 'profile' ? '' : 'border-transparent text-muted hover:text-body' ?>" style="<?= $activeTab === 'profile' ? 'border-color:var(--primary);color:var(--primary)' : ''">
    ...
    <a href="/account/orders" class="tab-btn px-5 py-3 text-sm font-medium rounded-t-lg transition -mb-px border-b-2 <?= $activeTab === 'orders' ? '' : 'border-transparent text-muted hover:text-body' ?>" style="<?= $activeTab === 'orders' ? 'border-color:var(--primary);color:var(--primary)' : ''">
      ...
      <?php if (count($orders) > 0): ?>
        <span class="ml-1 px-1.5 py-0.5 text-xs rounded-full bg-input"><?= count($orders) ?></span>
      <?php endif; ?>
    </a>
    <a href="/account/address" class="tab-btn px-5 py-3 text-sm font-medium rounded-t-lg transition -mb-px border-b-2 <?= $activeTab === 'address' ? '' : 'border-transparent text-muted hover:text-body' ?>" style="<?= $activeTab === 'address' ? 'border-color:var(--primary);color:var(--primary)' : ''">
    ...
    <a href="/account/wishlist" class="tab-btn px-5 py-3 text-sm font-medium rounded-t-lg transition -mb-px border-b-2 <?= $activeTab === 'wishlist' ? '' : 'border-transparent text-muted hover:text-body' ?>" style="<?= $activeTab === 'wishlist' ? 'border-color:var(--primary);color:var(--primary)' : ''">
      ...
      <?php if (!empty($wishlistProducts) && $activeTab !== 'wishlist'): ?>
        <span class="ml-1 px-1.5 py-0.5 text-xs rounded-full bg-input"><?= count($wishlistProducts) ?></span>
      <?php endif; ?>
    </a>
  </div>
  ...
  <div class="tab-content <?= $activeTab !== 'profile' ? 'hidden' : '' ?>" id="tab-profile">
    <div class="bg-card rounded-xl p-6 border border-color max-w-lg">
      <h2 class="font-semibold mb-4"><?= __('account_info') ?></h2>
      <div class="space-y-3 text-sm">
        <div class="flex justify-between py-2 border-b border-color">
          <span class="text-muted"><?= __('auth_name') ?></span>
          <span class="font-medium"><?= escape($user['name']) ?></span>
        </div>
        <div class="flex justify-between py-2 border-b border-color">
          <span class="text-muted"><?= __('auth_email') ?></span>
          <span class="font-medium"><?= escape($user['email']) ?></span>
        </div>
        <div class="flex justify-between py-2">
          <span class="text-muted"><?= __('account_member_since') ?></span>
    ...
    <?php else: ?>
      <div class="bg-card rounded-xl border border-color overflow-hidden">
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-color bg-page">
              <th class="text-left px-5 py-3 font-medium text-muted"><?= __('order_number_short') ?></th>
              <th class="text-left px-5 py-3 font-medium text-muted"><?= __('order_date') ?></th>
              <th class="text-left px-5 py-3 font-medium text-muted"><?= __('order_status') ?></th>
              <th class="text-right px-5 py-3 font-medium text-muted"><?= __('order_total') ?></th>
              <th class="text-right px-5 py-3 font-medium text-muted">Actions</th>
            </tr>
          </thead>
          <tbody>
            <?php foreach ($orders as $order): ?>
            <tr class="border-b border-color hover:bg-input transition">
              <td class="px-5 py-4 font-medium">#<?= $order['id'] ?></td>
              <td class="px-5 py-4 text-muted"><?= date('M j, Y', strtotime($order['created_at'])) ?></td>
              <td class="px-5 py-4">
                <span class="px-2.5 py-1 rounded-full text-xs font-semibold" style="background:var(--<?php 
                  if ($order['status'] === 'pending' || $order['status'] === 'paid'): ?>warning<?php 
  ...
  <div class="tab-content <?= $activeTab !== 'address' ? 'hidden' : '' ?>" id="tab-address">
    <div class="bg-card rounded-xl p-6 border border-color max-w-lg">
      ...
      <?php if ($user['address']): ?>
      <div id="address-display" class="text-sm space-y-1 mb-4 p-4 bg-page rounded-lg">
        <p class="font-medium"><?= escape($user['address']) ?></p>
        <p class="text-muted"><?= escape($user['city']) ?> <?= escape($user['zip']) ?></p>
      </div>
      <?php else: ?>
      <p id="address-display" class="text-sm text-muted italic mb-4"><?= __('account_no_address') ?></p>
      <?php endif; ?>

      <button onclick="toggleAddressForm()" class="btn-primary text-sm py-2 px-4"><?= __('admin_edit') ?></button>

      <form id="address-form" method="POST" action="/account/update-address" class="hidden space-y-4 mt-4 pt-4 border-t border-color">
        <div>
          <label class="block text-sm font-medium mb-1"><?= __('checkout_address') ?></label>
          <input type="text" name="address" value="<?= escape($user['address'] ?? '') ?>" class="w-full border border-input rounded-lg px-4 py-2 bg-card">
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium mb-1"><?= __('checkout_city') ?></label>
            <input type="text" name="city" value="<?= escape($user['city'] ?? '') ?>" class="w-full border border-input rounded-lg px-4 py-2 bg-card">
          </div>
          <div>
            <label class="block text-sm font-medium mb-1"><?= __('checkout_zip') ?></label>
            <input type="text" name="zip" value="<?= escape($user['zip'] ?? '') ?>" class="w-full border border-input rounded-lg px-4 py-2 bg-card">
          </div>
        </div>
        <div class="flex gap-2">
          <button type="submit" class="btn-primary text-sm py-2 px-4"><?= __('admin_save') ?></button>
          <button type="button" onclick="toggleAddressForm()" class="text-sm px-4 py-2 border border-input rounded-lg hover:bg-input"><?= __('admin_cancel') ?></button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="tab-content <?= $activeTab !== 'wishlist' ? 'hidden' : '' ?>" id="tab-wishlist">
    <?php if (empty($wishlistProducts)): ?>
      <div class="text-center py-16">
        <svg class="w-20 h-20 mx-auto mb-4 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
        <p class="text-lg text-muted"><?= __('account_no_wishlist') ?></p>
        <a href="/shop" class="btn-primary mt-4"><?= __('cart_start_shopping') ?></a>
      </div>
    <?php else: ?>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php foreach ($wishlistProducts as $item): ?>
          <div class="bg-card rounded-xl border border-color overflow-hidden group">
            ...
            <div class="p-4">
              ...
              <div class="flex items-center justify-between mb-3">
                <?php if ($item['sale_price']): ?>
                  ...
                  <span class="text-xs text-muted line-through"><?= $SETTINGS['currency'] ?><?= number_format($item['price'], 2) ?></span>
                <?php else: ?>
              ...
              <div class="flex gap-2">
                ...
                <button onclick="removeFromWishlist(<?= $item['id'] ?>)" class="px-3 py-2 border border-input rounded-lg transition" style="color:var(--error)" title="<?= __('admin_delete') ?>">
