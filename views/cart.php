<div class="max-w-4xl mx-auto px-4 py-8">
  ...
  <?php if (empty($cart)): ?>
    <div class="text-center py-16">
      ...
      <p class="text-lg text-muted mb-4"><?= __('cart_empty') ?></p>
      <a href="/shop" class="btn-primary"><?= __('cart_start_shopping') ?></a>
    </div>
  <?php else: ?>
    <div class="bg-card rounded-xl shadow-sm border border-color overflow-hidden">
      <table class="w-full">
        <thead class="bg-page text-sm">
          <tr>
            ...
            <tr>
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <?php if ($item['size'] || $item['color']): ?>
                      <p class="text-xs text-muted"><?= $item['size'] ? __('product_sizes') . ': ' . escape($item['size']) : '' ?><?= $item['size'] && $item['color'] ? ' | ' : '' ?><?= $item['color'] ? escape($item['color']) : '' ?></p>
                    <?php endif; ?>
                  </div>
                </div>
              </td>
              ...
              <td class="px-4 py-4 text-center">
                <form action="/cart" method="POST" class="inline-flex items-center gap-1">
                  ...
                  <button type="submit" name="quantity" value="<?= max(0, $item['quantity'] - 1) ?>" class="px-2 py-1 border border-input rounded hover:bg-input">−</button>
                  <span class="w-8 text-center"><?= $item['quantity'] ?></span>
                  <button type="submit" name="quantity" value="<?= $item['quantity'] + 1 ?>" class="px-2 py-1 border border-input rounded hover:bg-input">+</button>
                </form>
              </td>
              ...
              <td class="px-4 py-4 text-right">
                <form action="/cart" method="POST">
                  ...
                  <input type="hidden" name="index" value="<?= $index ?>">
                  <button type="submit" class="style="color:var(--error)"" title="<?= __('cart_remove') ?>">
    ...
    <div class="mt-8 flex flex-col md:flex-row justify-between items-start gap-6">
      ...
      <div class="w-full md:w-80 bg-card rounded-xl p-6 shadow-sm border border-color">
        <div class="space-y-2 text-sm">
          <div class="flex justify-between">
            <span class="text-muted"><?= __('cart_subtotal') ?></span>
            <span><?= $SETTINGS['currency'] ?><?= number_format($subtotal, 2) ?></span>
          </div>
          <div class="flex justify-between">
            <span class="text-muted"><?= __('cart_shipping') ?></span>
            <span>
              <?php if ($shipping == 0): ?>
                <span class="style="color:var(--success)" font-medium"><?= __('cart_free_shipping', ['min' => $SETTINGS['currency'] . number_format($SETTINGS['free_shipping_min'], 2)]) ?></span>
              <?php else: ?>
          ...
          <?php endif; ?>
          <div class="border-t border-color pt-2 flex justify-between font-bold text-lg">
        ...
        <div class="mt-4 pt-4 border-t border-color">
          ...
          <p id="coupon-msg" class="text-xs mt-1 <?= $couponCode ? 'style="color:var(--success)"' : '' ?>"><?= $couponCode && $couponDiscount > 0 ? 'Coupon applied!' : '' ?></p>
        </div>
    ...
    async function applyCoupon() {
      ...
      if (!code) { msg.textContent = 'Please enter a code.'; msg.className = 'text-xs mt-1 style="color:var(--error)"'; return; }
      try {
        ...
        if (data.success) {
          ...
          msg.className = 'text-xs mt-1 style="color:var(--error)"';
        }
      } catch(e) {
