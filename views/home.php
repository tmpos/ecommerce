...
<section class="carousel-container hero-gradient text-white">
  ...
  <div class="carousel-slide <?= $i === 0 ? 'active' : '' ?>" data-index="<?= $i ?>" style="min-height: 650px; <?= !empty($slide['image']) ? "background: url('" . escape($slide['image']) . "') center/cover no-repeat;" : '' ?>">
    <div class="text-center px-4 py-20" <?= !empty($slide['image']) ? "style='background: rgba(0,0,0,0.4); padding: 60px 24px; border-radius: 16px;'" : '' ?>>
      ...
      <a href="<?= escape($slide['btn_link'] ?? '/shop') ?>" class="inline-flex items-center gap-2 bg-white text-heading px-8 py-3 rounded-full font-semibold hover:bg-input transition">
...
<section class="hero-gradient text-white py-32" style="<?= $staticImage ? "background: url('" . escape($staticImage) . "') center/cover no-repeat;" : '' ?>">
  <div class="max-w-7xl mx-auto px-4 text-center" <?= $staticImage ? "style='background: rgba(0,0,0,0.4); padding: 60px 24px; border-radius: 16px; display: inline-block;'" : '' ?>>
    ...
    <a href="/shop" class="inline-flex items-center gap-2 bg-white text-heading px-8 py-3 rounded-full font-semibold hover:bg-input transition">
      <?= __('home_hero_btn') ?>
      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
    </a>
  </div>
...
<section class="max-w-7xl mx-auto px-4 mt-12">
  <h2 class="text-2xl font-bold mb-6 text-heading"><?= __('home_categories') ?></h2>
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    <?php foreach ($categories as $cat): ?>
      <a href="/shop?category=<?= escape($cat['slug']) ?>" class="product-card bg-card rounded-xl overflow-hidden shadow-sm border border-color">
        <div class="img-placeholder h-32 flex items-center justify-center text-lg font-semibold text-heading" style="background: linear-gradient(135deg, <?= $SETTINGS['primary_color'] ?>22, <?= $SETTINGS['secondary_color'] ?>22)">
          <?= escape($cat['name']) ?>
        </div>
        <div class="p-3 text-center font-medium text-heading"><?= escape($cat['name']) ?></div>
      </a>
    <?php endforeach; ?>
  </div>
</section>
...
<section class="max-w-7xl mx-auto px-4 mt-16">
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-bold text-heading"><?= __('home_promotions') ?></h2>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    <a href="/product/<?= escape($product['slug']) ?>" class="product-card bg-card rounded-xl overflow-hidden shadow-sm border border-color">
      <div class="img-placeholder h-48 flex items-center justify-center relative">
        ...
        <?php endif; ?>
        <span class="absolute top-2 left-2 text-white text-xs font-bold px-2 py-1 rounded" style="background:var(--primary)">-<?= round((1 - $product['sale_price'] / $product['price']) * 100) ?>%</span>
      </div>
      <div class="p-4">
        <p class="text-xs text-muted mb-1"><?= escape($product['category_name'] ?? '') ?></p>
        <h3 class="font-semibold mb-1 text-heading"><?= escape($product['name']) ?></h3>
...
<section class="max-w-7xl mx-auto px-4 mt-16">
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-bold text-heading"><?= __('home_new_collections') ?></h2>
...
<section class="max-w-7xl mx-auto px-4 mt-16">
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-bold text-heading"><?= __('home_featured') ?></h2>
    <a href="/shop" class="text-sm font-medium" style="color: var(--primary)"><?= __('shop_title') ?> &rarr;</a>
  </div>
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
...
<!-- ══════ BRANDS CAROUSEL ══════ -->
<section class="max-w-7xl mx-auto px-4 mt-16">
  <h2 class="text-2xl font-bold mb-6 text-center text-heading"><?= __('home_brands') ?></h2>
  <div class="flex flex-wrap justify-center items-center gap-8">
    ...
    <div class="grayscale hover:grayscale-0 transition duration-300">
      ...
      <?php else: ?>
        <span class="text-lg font-semibold text-muted"><?= escape($brand['name']) ?></span>
      <?php endif; ?>
    </div>
    <?php endforeach; ?>
  </div>
...
<!-- ══════ TESTIMONIALS ══════ -->
<section class="max-w-7xl mx-auto px-4 mt-16">
  <h2 class="text-2xl font-bold mb-6 text-center text-heading"><?= __('home_testimonials') ?></h2>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <?php foreach ($testimonials as $t): ?>
    <div class="bg-card rounded-xl p-6 shadow-sm border border-color">
      <div class="flex items-center gap-1 mb-3">
        <?php for ($i = 1; $i <= 5; $i++): ?>
          <svg class="w-4 h-4 <?= $i <= $t['rating'] ? 'text-yellow-400' : 'text-muted' ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
        <?php endfor; ?>
      </div>
      <p class="text-muted text-sm leading-relaxed mb-4">&ldquo;<?= escape($t['text']) ?>&rdquo;</p>
      <div class="flex items-center gap-3">
        <?php if (!empty($t['image'])): ?>
        <img src="<?= escape($t['image']) ?>" alt="<?= escape($t['name']) ?>" class="w-10 h-10 rounded-full object-cover">
        <?php else: ?>
        <div class="w-10 h-10 rounded-full bg-input flex items-center justify-center text-sm font-semibold text-muted"><?= mb_substr($t['name'], 0, 1) ?></div>
        <?php endif; ?>
        <span class="font-medium text-sm text-heading"><?= escape($t['name']) ?></span>
      </div>
    </div>
    <?php endforeach; ?>
  </div>
...
<section id="newsletter" class="max-w-7xl mx-auto px-4 mt-16">
  <div class="rounded-2xl p-12 text-center text-white" style="background: linear-gradient(135deg, <?= $SETTINGS['primary_color'] ?>, <?= $SETTINGS['secondary_color'] ?: $SETTINGS['primary_color'] ?>)">
    ...
    <form method="POST" action="/newsletter" class="flex max-w-md mx-auto gap-3">
      <input type="email" name="email" required placeholder="<?= __('home_newsletter_placeholder') ?>" class="flex-1 px-4 py-3 rounded-lg text-heading placeholder-gray-400 border-0 focus:ring-2 focus:ring-white/50">
...
<section class="max-w-7xl mx-auto px-4 mt-16">
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-bold text-heading"><?= __('home_gallery') ?></h2>
  ...
  <div class="text-center py-12 border-2 border-dashed border-input rounded-xl">
    ...
    <p class="text-sm text-muted"><?= __('home_gallery_empty') ?></p>
  </div>
  <?php endif; ?>
</section>
<?php endif; ?>
