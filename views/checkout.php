<div class="max-w-4xl mx-auto px-4 py-8">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    <div>
      ...
      <div class="space-y-4" id="shipping-fields">
        <div>
          ...
          <input type="text" id="address" value="<?= escape($user['address'] ?? '') ?>" required class="w-full border border-input rounded-lg px-4 py-2 bg-card">
        </div>
        <div>
          <label class="block text-sm font-medium mb-1"><?= __('checkout_city') ?></label>
          <input type="text" id="city" value="<?= escape($user['city'] ?? '') ?>" required class="w-full border border-input rounded-lg px-4 py-2 bg-card">
        </div>
        <div>
          <label class="block text-sm font-medium mb-1"><?= __('checkout_zip') ?></label>
          <input type="text" id="zip" value="<?= escape($user['zip'] ?? '') ?>" required class="w-full border border-input rounded-lg px-4 py-2 bg-card">
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="bg-card rounded-xl p-4 border border-color">
        ...
        <div class="border-t border-color mt-4 pt-4 space-y-2 text-sm">
          <div class="flex justify-between">
          ...
          <div class="flex justify-between font-bold text-lg pt-2 border-t border-color">
      ...
      <div id="stripe-wrapper" class="mt-6">
        ...
        <p id="stripe-error" class="style="color:var(--error)" text-sm mt-2 hidden"></p>
      </div>

      <div class="mt-4 text-center">
        <a href="/cart" class="text-sm text-muted hover:underline">&larr; <?= __('cart_back') ?></a>
      </div>
    </div>
  </div>
</div>
