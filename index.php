...
function handleNewsletter() {
    ...
    try {
        $stmt = $DB->prepare('INSERT OR IGNORE INTO newsletter_subscribers (email) VALUES (?)');
        $stmt->execute([$email]);
        if ($stmt->rowCount() > 0) {
...
function handleProduct($slug) {
    ...
    $related = $DB->prepare('SELECT * FROM products WHERE category_id = ? AND id != ? ORDER BY RANDOM() LIMIT 4');
...
function handleAdminSettings($method, $segments) {
        ...
        if ($action === 'migrate') {
            ...
            try {
                ...
                foreach ($tables as $tbl) {
                    ...
                    if ($targetDriver === 'mysql') {
                        $createSQL = "CREATE TABLE IF NOT EXISTS `$tbl` (" . implode(', ', $colDefs) . ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4";
                    }
    ...
    if ($method === 'POST') {
        ...
        $SETTINGS['favicon'] = $_POST['favicon'] ?? $SETTINGS['favicon'];
        ...
        // Hero
        $SETTINGS['home_hero_type'] = $_POST['home_hero_type'] ?? 'static';
